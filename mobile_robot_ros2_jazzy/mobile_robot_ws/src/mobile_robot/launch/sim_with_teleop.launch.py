"""
sim_with_teleop.launch.py
─────────────────────────────────────────────────────────────
Full Gazebo Harmonic simulation with keyboard teleop control.

Includes:
  • Gazebo Harmonic simulation
  • Robot spawned from URDF/xacro
  • ros2_control (differential drive + joint state broadcaster)
  • robot_state_publisher (TF tree)
  • LiDAR laser scan (visualized in RViz)
...
def generate_launch_description():
    ...
    teleop_node = Node(
        ...
        remappings=[
            ("/cmd_vel", "/cmd_vel")  # Remaps to /cmd_vel for diff_drive controller
        ],
        parameters=[{
            "use_sim_time": True,
        }],
    ...
    return LaunchDescription([
        ...
        teleop_node,  # Added keyboard teleop
        rviz2,
    ])
