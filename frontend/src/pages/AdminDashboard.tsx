...
export const AdminDashboard: React.FC = () => {
  ...
  const [qrMode, setQrMode] = useState<'tunnel' | 'local'>('tunnel');
    ...
    if (activeTab === 'inventory') {
      ...
      const generateQR = async () => {
        ...
        try {
          ...
          if (res && res.url) {
            tunnelUrl = `${res.url}/quick-request`;
          }
        } catch (e) {}

        try {
          const ipRes = await configApi.getNetworkIp();
          if (ipRes && ipRes.ip && ipRes.ip !== '127.0.0.1') {
            const port = window.location.port ? `:${window.location.port}` : '';
            localUrl = `${window.location.protocol}//${ipRes.ip}${port}/quick-request`;
          }
        } catch (e) {}

        if (!tunnelUrl) {
          tunnelUrl = `${window.location.protocol}//${window.location.host}/quick-request`;
        }
        if (!localUrl) {
          localUrl = `${window.location.protocol}//${window.location.host}/quick-request`;
        }

        const activeUrl = qrMode === 'tunnel' ? tunnelUrl : localUrl;
        setPortalUrlString(activeUrl);
      ...
      generateQR();
      const qrInterval = setInterval(generateQR, 8000);
              ...
              <div className="grid-cols-2" style={{ gap: '32px', marginBottom: '2rem' }}>
                ...
                <GlassPanel style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '32px', textAlign: 'center' }}>
                  ...
                  <p style={{ fontSize: '0.9rem', color: 'var(--text-secondary)', marginBottom: '24px', maxWidth: '300px' }}>
                    Scan this QR code with any mobile device to open the User Login & Equipment Ordering Portal.
                  </p>
                  
                  {/* Network Mode Switcher */}
                  <div style={{ display: 'flex', gap: '8px', marginBottom: '20px', background: 'rgba(0,0,0,0.3)', padding: '4px', borderRadius: '12px', border: '1px solid var(--glass-border)' }}>
                    <button
                      type="button"
                      className="btn"
                      ...
                      style={{
                        ...
                        fontSize: '0.8rem',
                        borderRadius: '8px',
                        background: qrMode === 'tunnel' ? 'var(--accent-cyan)' : 'transparent',
                        color: qrMode === 'tunnel' ? '#000' : 'var(--text-secondary)',
                        fontWeight: 600
                      }}
                    >
                      🌐 Cloudflare Tunnel
                    ...
                    <button
                      ...
                      style={{
                        ...
                        fontSize: '0.8rem',
                        borderRadius: '8px',
                        background: qrMode === 'local' ? 'var(--accent-green)' : 'transparent',
                        color: qrMode === 'local' ? '#000' : 'var(--text-secondary)',
                        fontWeight: 600
                      }}
                    >
                      📶 Wi-Fi Network IP
                  ...
                  <div style={{ display: 'flex', gap: '8px', width: '100%', maxWidth: '340px' }}>
                    ...
                    <a 
                      href={portalUrlString || "/quick-request"} 
                      target="_blank" 
                      rel="noreferrer"
                      className="btn" 
                ...
                <GlassPanel style={{ padding: '32px', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
                  ...
                  <h3 style={{ margin: '0 0 12px 0' }}>User Node & Mail OTP Portal</h3>
                  <p style={{ fontSize: '0.95rem', color: 'var(--text-secondary)', lineHeight: '1.6', marginBottom: '16px' }}>
                    Scanning the QR code directs students and researchers to the User Portal where they authenticate using their registered Mail ID.
                  </p>
                  <ul style={{ fontSize: '0.9rem', color: 'var(--text-secondary)', paddingLeft: '20px', display: 'flex', flexDirection: 'column', gap: '8px', margin: 0 }}>
                    <li><strong>Mail ID Authentication:</strong> Users log in securely with their email address.</li>
                    <li><strong>Streamlined Ordering:</strong> Browse available equipment and dispatch the robot to their lab station.</li>
                    <li><strong>Mail OTP Verification:</strong> When the robot arrives, an OTP is emailed to the user. Entering the OTP unlocks the physical flap.</li>
                  </ul>
                </GlassPanel>
              </div>
